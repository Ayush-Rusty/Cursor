...
const ContinueCard=({data}:{data:Doc<"projects">})=>{
    return(
        <div className="flex flex-col gap-2">
            ...
            <Button variant="outline" className="h-auto items-start justify-start p-4 bg-background border rounded-none flex flex-col gap-2">
...
const ProjectItem=({data}:{data:Doc<"projects">})=>{
    ...
    <Link 
        ...
        className="text-sm text-foreground/60 font-medium hover:text-foreground py-1 items-center justify-between w-full group"
        >
            <div className="flex items-center gap-2">
                {getProjectIcon(data)}
                <span className="truncate">{data.name}</span>
            </div>
            <span className="text-xs text-mutes-foreground group-hover:text-foreground/60 transition-colors">
